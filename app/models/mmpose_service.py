...
import cv2
import numpy as np
# import supervision as sv # Not used for drawing in this version
...
logger.info("--- MMPOSE SERVICE SCRIPT STARTED (Explicit Paths, URL-Only, FFMPEG, Detailed Logging) ---")

app = FastAPI(title="MMPose Biomechanics Service (URL Input Only)", version="1.2.0") # Incremented version
...
try:
    ...
    rtmpose_config_path = '/opt/padel/app/configs/body_2d_keypoint/rtmpose/coco/rtmpose-m_8xb256-420e_coco-256x192.py'
    rtmpose_checkpoint_path = '/opt/padel/app/weights/rtmpose-m_simcc-aic-coco_pt-aic-coco_420e-256x192-63eb25f7_20230126.pth'
    # If you prefer init_model to download the checkpoint, use the URL:
    # rtmpose_checkpoint_url = 'https://download.openmmlab.com/mmpose/v1/projects/rtmpose/rtmpose-m_simcc-aic-coco_pt-aic-coco_420e-256x192-63eb25f7_20230126.pth'

    logger.info(f"Attempting to load RTMPose-M COCO model from explicit paths:")
    logger.info(f"Config: {rtmpose_config_path}")
    logger.info(f"Checkpoint: {rtmpose_checkpoint_path}") # Or rtmpose_checkpoint_url

    if os.path.exists(rtmpose_config_path):
        mmpose_model = init_model(
            rtmpose_config_path,
            checkpoint=rtmpose_checkpoint_path, # Use local path if downloaded by deploy.yml
            # checkpoint=rtmpose_checkpoint_url, # Or use URL to let init_model download
            device=model_device
        )
        logger.info("RTMPose model loaded successfully.")
    else:
        logger.warning(f"RTMPose config file not found at {rtmpose_config_path}. This will lead to fallback.")
        raise FileNotFoundError(f"Primary RTMPose config not found: {rtmpose_config_path}")

except Exception as e_rtmpose:
    logger.warning(f"Could not load primary RTMPose model: {e_rtmpose}. Attempting HRNet fallback.", exc_info=True)
    try:
        model_device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
        
        hrnet_config_path = '/opt/padel/app/configs/body_2d_keypoint/topdown_heatmap/coco/td-hm_hrnet-w48_8xb32-210e_coco-256x192.py'
        hrnet_checkpoint_path = '/opt/padel/app/weights/hrnet_w48_coco_256x192-b9e0b3ab_20200708.pth'
        # Or URL for HRNet checkpoint:
        # hrnet_checkpoint_url = 'https://download.openmmlab.com/mmpose/top_down/hrnet/hrnet_w48_coco_256x192-b9e0b3ab_20200708.pth'

        logger.info(f"Attempting to load HRNet-W48 COCO model as fallback from explicit paths (device: {model_device}):")
        logger.info(f"Config: {hrnet_config_path}")
        logger.info(f"Checkpoint: {hrnet_checkpoint_path}") # Or hrnet_checkpoint_url

        if os.path.exists(hrnet_config_path):
            mmpose_model = init_model(
                hrnet_config_path,
                checkpoint=hrnet_checkpoint_path, # Use local path
                # checkpoint=hrnet_checkpoint_url,   # Or use URL
                device=model_device
            )
            logger.info("HRNet fallback model loaded successfully.")
        else:
            logger.error(f"HRNet fallback config file not found at {hrnet_config_path}. Cannot load any MMPose model.")
            raise FileNotFoundError(f"Fallback HRNet config not found: {hrnet_config_path}")
    except Exception as e_hrnet:
        logger.error(f"CRITICAL: Failed to load any MMPose model (RTMPose primary or HRNet fallback): {e_hrnet}", exc_info=True)
        mmpose_model = None
...
def calculate_angle(a, b, c):
    ...
    if magnitude_product < 1e-7: return 0.0
    cos_angle = np.clip(dot_product / magnitude_product, -1.0, 1.0)
    angle = np.degrees(np.arccos(cos_angle))
    return angle

def calculate_posture_score(keypoints): return np.random.uniform(70,95) if keypoints else 0.0
def calculate_balance_score(keypoints): return np.random.uniform(65,90) if keypoints else 0.0
def calculate_movement_efficiency(joint_angles): return np.random.uniform(60,95) if joint_angles else 0.0
def calculate_power_potential(joint_angles, keypoints): return np.random.uniform(50,100) if joint_angles and keypoints else 0.0
...
def analyze_frame_biomechanics(frame_content: np.ndarray) -> Dict[str, Any]:
    if mmpose_model is None:
        ...
        return {"keypoints": {}, "joint_angles": {}, "biomechanical_metrics": {"error_processing_frame": True, "posture_score": 0}}
    ...
    current_keypoints, joint_angles, biomechanical_metrics = {}, {}, {}
    ...
    try:
        ...
        if pose_data_samples:
            ...
            if hasattr(data_sample, 'pred_instances') and data_sample.pred_instances and len(data_sample.pred_instances) > 0:
                ...
                pred_kpts, pred_scores = pred_kpts_tensor.cpu().numpy(), pred_scores_tensor.cpu().numpy()
                for idx in range(pred_kpts.shape[0]):
                    if idx < len(keypoint_names):
                        current_keypoints[keypoint_names[idx]] = {"x": float(pred_kpts[idx,0]), "y": float(pred_kpts[idx,1]), "confidence": float(pred_scores[idx])}
        
        # Simplified angle calculations for brevity - use your full set
        if all(k in current_keypoints for k in ["left_shoulder", "left_elbow", "left_wrist"]):
            joint_angles["left_elbow"] = calculate_angle( (current_keypoints["left_shoulder"]["x"], current_keypoints["left_shoulder"]["y"]), (current_keypoints["left_elbow"]["x"], current_keypoints["left_elbow"]["y"]), (current_keypoints["left_wrist"]["x"], current_keypoints["left_wrist"]["y"]) )
        # ... (add all your other angle calculations here) ...

        biomechanical_metrics = {"posture_score": calculate_posture_score(current_keypoints), "balance_score": calculate_balance_score(current_keypoints), "movement_efficiency": calculate_movement_efficiency(joint_angles), "power_potential": calculate_power_potential(joint_angles, current_keypoints)}
...
def draw_biomechanics_on_frame(frame: np.ndarray, analysis: Dict[str, Any]) -> np.ndarray:
    ...
    for name, data in keypoints.items():
        ...
        if conf > 0.5: cv2.circle(annotated_frame, (x,y), 5, (0,int(255*conf), int(255*(1-conf))), -1)
    for p1_name, p2_name in connections:
        ...
        if p1 and p2 and p1.get("confidence",0)>0.5 and p2.get("confidence",0)>0.5:
            cv2.line(annotated_frame, (int(p1["x"]), int(p1["y"])), (int(p2["x"]), int(p2["y"])), (0,255,0), 2) # Simple green lines
    # Draw metrics & angles (simplified from your original for brevity, paste your full logic)
    y_offset = 30
    for name, val in metrics.items():
        cv2.putText(annotated_frame, f"{name}: {val}", (10,y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255),1)
        y_offset+=20
...
async def analyze_video_endpoint(
    ...
    try:
        ...
        logger.info(f"Finished biomechanics analysis. Analyses: {len(all_analyses_per_frame)}. Annotated: {len(annotated_frames_list)}.")
        ...
        if payload.video:
            if not annotated_frames_list:
                ...
                response_content["video_url"] = None; response_content["message"] = "MMPose: No frames to annotate."
            else:
                ...
                try:
                    ...
                    for frame_to_write in annotated_frames_list:
                        if process.stdin and process.stdin.closed: logger.warning("MMPose FFMPEG: stdin closed."); break
                        try:
                            if process.stdin: process.stdin.write(frame_to_write.tobytes())
                        except (IOError, BrokenPipeError) as e_pipe: logger.warning(f"MMPose FFMPEG: pipe broken: {e_pipe}"); break
                    ...
                    try:
                        ...
                        logger.error("MMPose FFMPEG: timed out. Killing."); process.kill()
                        ...
                        if "flush of closed file" in str(ve).lower() or "write to closed file" in str(ve).lower():
                            ...
                            except subprocess.TimeoutExpired: logger.warning("MMPose FFMPEG: timed out on wait().")
                            ffmpeg_return_code = process.poll()
                        else: logger.error(f"MMPose FFMPEG: ValueError: {ve}", exc_info=True); ffmpeg_return_code = -1
                    except Exception as e_comm: logger.error(f"MMPose FFMPEG: communicate() error: {e_comm}", exc_info=True); ffmpeg_return_code = -1
                    
                    if ffmpeg_return_code is None and not ffmpeg_timed_out: ffmpeg_return_code = process.poll()
                    if ffmpeg_timed_out: response_content["message"] = "MMPose FFMPEG: timed out."; gcs_url_result = ""
                        ...
                        gcs_url_result = ""; response_content["message"] = f"MMPose FFMPEG: failed. RC: {ffmpeg_return_code}"
                    else: logger.info(f"MMPose: Video via FFMPEG: {output_video_path}"); gcs_url_result = await upload_to_gcs(output_video_path)
                    response_content["video_url"] = gcs_url_result if gcs_url_result else None
                    if not gcs_url_result and "message" not in response_content: response_content["message"] = "MMPose: Upload fail/FFMPEG silent fail."
                finally:
                     if output_video_path and os.path.exists(output_video_path): os.unlink(output_video_path)
        
        if not response_content: return JSONResponse(content={"detail": "MMPose: No output."}, status_code=200)
        return JSONResponse(content=response_content)
            
    except HTTPException: raise
    except httpx.HTTPStatusError as e: logger.error(f"MMPose HTTP error: {e.response.status_code}", exc_info=True); raise HTTPException(status_code=400, detail=f"MMPose: DL fail: {e.response.status_code}")
    except Exception as e: logger.error(f"MMPose: Unexpected error: {e}", exc_info=True); raise HTTPException(status_code=500, detail=f"MMPose: Internal error: {str(e)}")
    finally:
