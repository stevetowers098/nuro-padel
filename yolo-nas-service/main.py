...
if SUPER_GRADIENTS_AVAILABLE:
    try:
        ...
        try:
            ...
            logger.info("Successfully loaded YOLO-NAS pose model")
        except Exception as e_pose:
            logger.error(f"Failed to load YOLO-NAS pose model: {e_pose}")
            
        # Load YOLO-NAS Object Detection Model
        try:
            ...
            logger.info("Successfully loaded YOLO-NAS object detection model")
        except Exception as e_object:
            logger.error(f"Failed to load YOLO-NAS object model: {e_object}")
            
        if yolo_nas_pose_model or yolo_nas_object_model:
            model_info["status"] = "partial_success"
        else:
            model_info["status"] = "failed"
            
    except Exception as e:
        logger.error(f"Failed to load YOLO-NAS models: {e}")
        model_info["status"] = "failed"
else:
    logger.error("Super Gradients not available - service will be unhealthy")
