...
try:
    ...
    class FallbackConfigLoader:
        ...
        def get_service_info(self): return {"service": "yolo8", "version": "1.0.0"}
...
logger.info("--- YOLO8 SERVICE STARTED ---")

# Initialize configuration loader
config_loader = ConfigLoader("yolo8", "/app/config")
...
app = FastAPI(title="YOLO8 Service - Object & Pose Detection", version="1.0.0")
logger.info("FastAPI app created for YOLO8 service.")
...
logger.info(f"Model files configured: YOLOv8 Object={YOLOV8_OBJECT_MODEL}, YOLOv8 Pose={YOLOV8_POSE_MODEL}")
...
async def health_check():
    ...
    models_status = {
        "yolov8_object": {
            "loaded": yolov8_object_model is not None,
            "enabled": current_config.get("models", {}).get("yolov8_object", {}).get("enabled", True),
            "file": YOLOV8_OBJECT_MODEL
        },
        "yolov8_pose": {
            "loaded": yolov8_pose_model is not None,
            "enabled": current_config.get("models", {}).get("yolov8_pose", {}).get("enabled", True),
            "file": YOLOV8_POSE_MODEL
        }
    }
...
async def process_pose_detection(payload: VideoAnalysisURLRequest, model: YOLO,
    ...
    """YOLOv8 pose detection processing logic"""
...
async def process_object_detection(payload: VideoAnalysisURLRequest, model: YOLO,
                                 service_name: str, gcs_folder: str):
    """Enhanced YOLOv8 object detection with ball tracking"""
    temp_downloaded_path = None
    try:
        # Download video
        async with httpx.AsyncClient(timeout=60.0) as client:
